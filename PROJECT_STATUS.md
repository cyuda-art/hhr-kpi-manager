...
2. **モック（ダミー環境・データ）は作成せず、常に本番環境を前提に開発する**
   * データベース（PostgreSQL / Prisma）やAPI連携の処理において、フェイルセーフとしてモックや一時的なLocalStorageに逃げるロジックを安易に追加しないこと。本番データベース（Supabase等）が正しく動作することを前提とし、エラーが出る場合はモックで誤魔化すのではなく根本的な原因（DBスキーマ、接続情報、マイグレーションなど）を解決すること。
