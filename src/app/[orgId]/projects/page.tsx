    ...
    <OrgLayout>
      ...
      {isCreating && (
        <div className="fixed inset-0 bg-[#000000]/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#282a2d] rounded-[8px] p-6 w-full max-w-md shadow-2xl border border-slate-200 dark:border-[#3c4043]">
            <h2 className="text-[20px] font-normal text-slate-900 dark:text-[#f1f3f4] mb-6">新しいプロジェクトを作成</h2>
            <form onSubmit={handleCreateProject} className="space-y-5">
              <div>
                <label className="block text-[13px] font-medium text-slate-500 dark:text-[#9aa0a6] mb-1.5">プロジェクト名 <span className="text-rose-500 dark:text-[#f28b82]">*</span></label>
                <input
                  type="text"
                  required
                  autoFocus
                  ...
                  className="w-full px-3 py-2 bg-slate-50 dark:bg-[#202124] text-slate-800 dark:text-[#e8eaed] border border-slate-300 dark:border-[#5f6368] focus:border-primary-500 dark:border-[#8ab4f8] rounded-[4px] focus:outline-none transition-colors text-[14px]"
                />
              </div>
              <div>
                <label className="block text-[13px] font-medium text-slate-500 dark:text-[#9aa0a6] mb-1.5">プロジェクトの説明 (任意)</label>
                <textarea
                  value={newProjectDesc}
                  onChange={(e) => setNewProjectDesc(e.target.value)}
                  placeholder="プロジェクトの目的や概要を入力してください"
                  rows={3}
                  className="w-full px-3 py-2 bg-slate-50 dark:bg-[#202124] text-slate-800 dark:text-[#e8eaed] border border-slate-300 dark:border-[#5f6368] focus:border-primary-500 dark:border-[#8ab4f8] rounded-[4px] focus:outline-none transition-colors text-[14px] resize-none"
                />
              </div>
              <div className="flex justify-end gap-2 pt-4">
                <button
                  ...
                  className="px-4 py-2 text-[14px] font-medium text-primary-600 dark:text-[#8ab4f8] hover:bg-primary-600 dark:bg-[#8ab4f8]/10 rounded-[4px] transition-colors"
                >
                  キャンセル
                </button>
                <button
                  ...
                  className="px-4 py-2 text-[14px] font-medium bg-primary-600 dark:bg-[#8ab4f8] text-white dark:text-[#202124] hover:bg-primary-700 dark:bg-[#aecbfa] disabled:opacity-50 disabled:hover:bg-primary-600 dark:bg-[#8ab4f8] rounded-[4px] transition-colors"
                >
                  続行
                </button>
              </div>
