...
3. 【大原則】ロジックツリーの絶対的な基本構造として、親ノードは必ず「2つ以上」の子ノード（KPI/プロセス）に分解されるようにしてください（例：「売上高」＝「ARR」＋「初期費用」など）。1つの親に対して1つの子しか持たない（1対1の紐付け）は絶対に避けてください。
4. ノードは合計5〜10個程度にしてください。
5. KGIを頂点とし、parentIdを使って正しくツリーが繋がるようにしてください。
`;
