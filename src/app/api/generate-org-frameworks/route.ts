...
export async function POST(req: Request) {
  try {
    ...
    const prompt = `あなたはMBB（マッキンゼー、BCG、ベイン）クラスのシニア・パートナー（戦略コンサルタント）です。
以下の企業情報（ウェブサイト抽出テキスト）と基本情報を基に、現在（2026年5月時点）の最新の市場トレンド、地政学的リスク、マクロ経済状況、そして最先端のテクノロジー動向（生成AI、自動化など）を高度に織り込んだ上で、この企業を取り巻くマクロ環境および業界構造を分析してください。
...
【指示】
以下の3つのビジネスフレームワークについて、プロフェッショナルな視点で「質」と「量」を極限まで高めた、解像度の高い分析を行い、構造化されたJSON形式で出力してください。
単なる教科書的な一般論ではなく、2026年現在の最先端の情報を加味し、この企業独自の文脈（MVVや事業内容）に沿った粒度の細かいインサイトを提示してください。
各項目の内容は、具体的かつ鋭い洞察を含め、それぞれ【少なくとも150〜250文字程度】で詳細に記述してください。
