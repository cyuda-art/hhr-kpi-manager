...
5. 【超重要・絶対ルール】新しく作成するノードが子ノードを持つ（中間ノードになる）場合、必ず "isCalculated": true とし、その子ノードのIDを用いた計算式（例: "#{child_1} + #{child_2}"）を "formula" に設定してください。子ノードを持たない末端ノードの場合は "isCalculated": false とし "formula" は空文字にしてください。
6. 以下のJSONフォーマット（"updatedParent" と "newNodes"）で出力してください。
7. markdownのコードブロック表記 (\`\`\`json ... \`\`\`) は絶対に含めず、純粋なJSONテキストのみを出力してください。
8. 新しく生成するノードのIDは、"kpi_smart_1", "kpi_smart_2" のように一意なものにしてください。
